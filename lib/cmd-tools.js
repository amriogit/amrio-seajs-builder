...
var cmdTools = {
    ...
    parseMeta: function(codeAst) {
        ...
        codeAst.walk(new UglifyJS.TreeWalker(function(node) {
            ...
            if (cmdTools.isValidDefine(node)) {
    ...
    collectDependencies: function(factoryAst) {
        ...
        function handle(node) {
            ...
            if (cmdTools.isValidDefine(node) === false) {
    ...
    beautify: function (codeAst, isJustPrintToString) {
        return codeAst.print_to_string({
            beautify: isJustPrintToString ? false : true
        })
    }
}
