...
var cmdTools = {
    ...
    extractMeta: function(node) {
        ...
        if (args.length === 1) {
            if (args[0] instanceof UglifyJS.AST_Function) {
                meta.deps = this.collectDependencies(args[0])
            }
        }
        ...
        // define(id || deps, factory)
            ...
            if (args[0] instanceof UglifyJS.AST_String) {
                ...
                if (args[1] instanceof UglifyJS.AST_Function) {
                    meta.deps = this.collectDependencies(args[1])
                }
            }
    ...
    minify: function(codeAst, isMinify, options) {
        options = _.assign({
            beautify: {
                beautify: true,
                ascii_only: false
        ...
        if (isMinify) {
            codeAst = codeAst.transform(UglifyJS.Compressor(options.compress))
            codeAst.figure_out_scope()
        ...
        return codeAst.print_to_string(options.beautify)
    }
}
