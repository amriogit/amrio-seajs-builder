...
helper.extend(Module.prototype, {
    ...
    concat: function() {
        ...
        function findAllFactories(mods) {
            mods.forEach(function(mod) {
                if (!cache[mod.meta.id]) {

                    cache[mod.meta.id] = true
                    allFactories.push(mod.factory)

                    if (mod.depsMods.length) {
                        findAllFactories(mod.depsMods)
                    }
                }
            })
        }

        findAllFactories(this.depsMods)

        this.result = [this.factory].concat(allFactories).join(this.options.footer)
    }
})
