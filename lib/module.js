...
helper.extend(Module.prototype, {
    ...
    fetch: function() {
        ...
        if (this.meta.factory === null) {
            helper.log(this.options.log, util.format('not found mod %s', chalk.yellow(this.meta.uri)))
            return
        }
    ...
    load: function() {
        ...
        if (notFoundDeps.length) {
            helper.log(this.options.log, util.format('module %s not found deps %s', chalk.cyan(this.meta.id), chalk.red('[' + notFoundDeps.join(', ') + ']')))
        }
